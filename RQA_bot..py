...
from langchain.vectorstores import Qdrant, Pinecone  # Importing Qdrant and Pinecone for vector storage
...
def get_vectorestore(COLLECTION, chunk_list):
    ...
    try:
        # Create a vector store using Pinecone with the provided parameters
        base = Pinecone.from_documents(
            documents=chunk_list,          # List of document chunks to be stored
            embedding=embeddings,          # Embedding model used for representing the documents
            api_key=pinecone_api_key,      # API key for Pinecone service
            url=pinecone_url,              # URL of the Pinecone service
            prefer_grpc=True,              # Use gRPC protocol for communication, if preferred
            collection_name=COLLECTION,    # Name of the collection to be created in Pinecone
        )
    # Catch any exceptions during the Pinecone initialization
    except Exception as e:
        # Write the error message to the Streamlit interface
        st.write(f'Error: {e}')

    # Return the Pinecone vector store object
    return base
